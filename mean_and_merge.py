...
for patient in patient_list:
  for cote in cotes:
    for metric in metrics:
      ...
      mean_dic[patient][cote][metric] = global_mean
      mean_dic[patient][cote]["n"] = global_n
      dev_dic[patient][cote][metric] = global_std
      

json.dump(mean_dic, open(root + "unravel_mean.json", 'w'), default=to_float64)
json.dump(dev_dic, open(root + "unravel_dev.json", 'w'), default=to_float64)      
